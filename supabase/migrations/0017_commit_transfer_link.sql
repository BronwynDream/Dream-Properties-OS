-- ============================================================================
-- Dream Knysna OS — 0017 commit_batch: honour explicit transfer.id from picker
-- ----------------------------------------------------------------------------
-- Root fix for the "one listing, three transfer rows" pattern hit on
-- 15 Eagles Way (2026-07-06). When a batch links to a property that already
-- has transfers, the reviewer can now pick one and commit into it instead of
-- creating a new one. commit_batch reads p_fields.transfer.id, verifies the
-- transfer belongs to the linked property (defensive — never let a batch
-- attach to some random transfer on another property), and uses it.
-- Everything else (parties, agreements, milestones, documents) flows into
-- the chosen transfer instead of a fresh row.
--
-- Backward compatible: if fields.transfer.id is not set, behaviour is
-- identical to 0016.
-- ============================================================================

-- Extend match_candidate.target_kind to include 'transfer' — a match_candidate
-- row with extracted_ref='transfer' now carries the reviewer's transfer pick.
alter table match_candidate drop constraint if exists match_candidate_target_kind_check;
alter table match_candidate
  add constraint match_candidate_target_kind_check
  check (target_kind in ('property','party','transfer'));
...
begin
  ...
  -- Transfer resolution: prefer an explicit picked transfer_id, but guard
  -- against attaching to a transfer on a different property. If the picked
  -- id doesn't belong to v_prop, silently fall through to creating a new one.
