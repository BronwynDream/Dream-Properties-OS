-- ============================================================================
-- Dream Knysna OS — 0035 tighten registered-advance rule
-- ----------------------------------------------------------------------------
-- Migration 0018 auto-advanced transfer.status to 'registered' if ANY file
-- in the batch was classified as a title_deed. Too aggressive: mandates
-- routinely ship with the previous owner's title deed as chain-of-title
-- reference material — that's not evidence that THIS transfer has registered.
--
-- Real-world misfire (St James Hotel, Jul 2026): batch of "Old Title Deeds.PDF"
-- + "SG Diagram.pdf" (misclassified as title_deed) + mandate → auto-registered
-- the listing before it had sold. Wrong status shown across the app.
--
-- New rule: require BOTH signals — a title_deed doc AND an extracted agreement
-- (price populated). An executed agreement + a title deed is a completed
-- registration cycle; a title deed alone is reference material.
--
-- Nothing else changes about commit_batch. We just guard the final status
-- update with the additional `v_price is not null` check.
-- ============================================================================
...
begin
  ...
  if v_prop_explicit is not null then
    begin
      v_prop := v_prop_explicit::uuid;
    exception when others then v_prop := null;
    end;
  end if;
  ...
  if v_prop is null and v_erf is not null then
    ...
    from property p join erf e on e.property_id = p.id
    where e.erf_number = v_erf limit 1;
  end if;
  if v_prop is null and v_addr is not null then
    select id into v_prop from property
    where primary_address ilike v_addr order by created_at limit 1;
  end if;

  if v_prop is null then
    if v_suburb is not null then
      select id into v_suburb_id from suburb where name ilike v_suburb limit 1;
    end if;
    if v_prop_type is not null then
      select id into v_prop_type_id from property_type where code = v_prop_type limit 1;
    end if;
    if v_own_type is not null then
      select id into v_own_type_id from ownership_type where code = v_own_type limit 1;
    end if;
    ...
    update property set
      primary_address = coalesce(v_addr, primary_address),
      title_deed_no   = coalesce(v_deed, title_deed_no),
      extent_sqm      = coalesce(v_extent, extent_sqm)
    where id = v_prop;
    if v_suburb is not null then
      select id into v_suburb_id from suburb where name ilike v_suburb limit 1;
      update property set suburb_id = coalesce(suburb_id, v_suburb_id) where id = v_prop;
    end if;
    if v_prop_type is not null then
      select id into v_prop_type_id from property_type where code = v_prop_type limit 1;
      update property set property_type_id = coalesce(property_type_id, v_prop_type_id) where id = v_prop;
    end if;
    if v_own_type is not null then
      select id into v_own_type_id from ownership_type where code = v_own_type limit 1;
      update property set ownership_type_id = coalesce(ownership_type_id, v_own_type_id) where id = v_prop;
    end if;
  end if;
  ...
  if v_transfer_explicit is not null then
    begin
      v_transfer := v_transfer_explicit::uuid;
    exception when others then v_transfer := null;
    end;
    if v_transfer is not null then
      perform 1 from transfer where id = v_transfer and property_id = v_prop;
      if not found then v_transfer := null; end if;
    end if;
  end if;
  ...
  -- Title-deed evidence + an executed agreement (price extracted) → deal has
  -- registered at the Deeds Office. Fires after any 'in_conveyancing'
  -- assignment above so the final status wins.
  -- Requires BOTH signals to avoid the misfire where a listing includes the
  -- previous owner's title deed as reference material.
  select exists (
