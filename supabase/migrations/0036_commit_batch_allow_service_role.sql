-- ============================================================================
-- Dream Knysna OS — 0036 allow service_role to call commit_batch
-- ----------------------------------------------------------------------------
-- commit_batch (migration 0018 → tightened in 0035) guards with:
--   if not coalesce(is_admin(), false) then raise 'not authorised to commit';
-- That's correct for reviewer sessions in the app, but blocks the intake
-- webhook — which uses a service-role client and has no auth session, so
-- is_admin() returns null and the check fails.
--
-- We want the webhook to auto-commit high-confidence batches (task #25 b)
-- so the property record populates end-to-end without a human click. Loosen
-- the guard to also allow the postgres service_role.
--
-- Nothing else about commit_batch changes.
-- ============================================================================
...
begin
  -- Allow reviewer sessions (is_admin) OR the intake webhook (service_role).
  -- Anon and authenticated non-admin sessions still can't commit.
  if not (
  ...
  -- registered at the Deeds Office. Requires BOTH signals to avoid the misfire
  -- where a listing includes the previous owner's title deed as reference.
  select exists (
